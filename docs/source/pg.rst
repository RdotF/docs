...
В Proxmox найдите сервер с идентификатором:
...
В этом разделе должна присутствовать учётная запись, которая будет использоваться для подключения через pgAdmin4.
...
Учётная запись создаётся пользователем, у которого уже есть доступ к PostgreSQL.
...
Для этого выполните SQL-команду:
...
.. image:: images/pg-2.png
   :alt: Настройка подключения в pgAdmin4
   :align: center
   :width: 700px

Для подключения к удалённому PostgreSQL не требуется устанавливать PostgreSQL на локальный компьютер.
...
В разделе **Connection** необходимо указать данные для подключения, предусмотренные на сервере:
...
В качестве пользователя укажите созданную учётную запись, например:
